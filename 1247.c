...
#define PIN_18B20 P35  //DS18B20通信引脚
...
code uchar cmdTCP[] = "AT+CIPSTART=\"TCP\",\"eeec.hpu.edu.cn\",7103\r\n";	//建立TCP连接
code uchar cmdTCPLen[] = "AT+CIPSEND=100\r\n";	//TCP请求长度
uchar cmdTCPURL[] = "GET /index.php?tip=mon&mac=00-00-00-00-00-00&T=00.0&S=000&H=000 HTTP/1.1\r\n Host: eeec.hpu.edu.cn\r\n\r\n";	//发起请求

uchar String[245] = "";	
uchar posString = 0, lenString = 0;
...
void main(){
	...
	while(1){
		getData();
		delayMs(20);delayMs(20);delayMs(20);delayMs(20);delayMs(20);
		delayMs(20);delayMs(20);delayMs(20);delayMs(20);delayMs(20); //延时20s
		S2BUF = failT+'0';DelayCharUsart();
		if(connected){
			failT++;
			if(failT >= 5){	//5次查询未接收到大于200个字符的信息则进行重启
				connected = 0;	//改变连接状态
				Beep = 1;
				delayMs(20);
				Beep = 0;
...
void getData(){		//模块查询数据
	...
	delayMs(10);//延时1S
	if(lenString>200){
		failT = 0;		//收到超过200个字符的信息就将失败次数置零
	}	
	/////////////////////////////String[236]:off[_]on[o]	
}
